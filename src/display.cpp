...
constexpr glm::vec3 TERRAIN_LOD_COLORS[] = {
    ...
    glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 1.0f),
...
namespace gfx {
    ...
    void displayWater(float totalTime) {
        ...
        waterShader.uniformMat4x4("persp", window.getPerspective());
    ...
    void generateDecorationOffsets(infworld::DecorationTable &decorations) {
        decorations.generateOffsets(infworld::PINE_TREE, VAOS->getVao("pinetree"), 0,
                                    4);
        decorations.generateOffsets(infworld::PINE_TREE,
                                    VAOS->getVao("pinetreelowdetail"), 4, 999);
    ...
    void displayDecorations(infworld::DecorationTable &decorations,
        ...
        treeShader.uniformMat4x4(
            "transform", glm::scale(glm::mat4(1.0f), glm::vec3(SCALE * 2.5f)));
    ...
    unsigned int displayTerrain(infworld::ChunkTable *chunktables, int maxlod,
                                float lodscale) {
        Window &window = Window::getInstance();
        Camera &cam = window.getCamera();
        ...
        // uniforms
        terrainShader.uniformMat4x4("persp", window.getPerspective());
        ...
        geo::Frustum viewfrustum =
                cam.getViewFrustum(window.getZnear(), window.getZfar(),
    ...
    void displayExplosions(const std::vector<gobjs::Explosion> &explosions) {
        ...
        shader.uniformMat4x4("persp", window.getPerspective());
        shader.uniformMat4x4("view", window.getCamera().viewMatrix());
  ...
  void displayHangar(float angle) {
        Window &window = Window::getInstance();
        Camera &cam = window.getCamera();
        ...
        const auto& groups = VAOS->getMaterialGroups("hangar");
        if (groups.empty()) {
            TEXTURES->bindTexture("hangar", GL_TEXTURE0);
            VAOS->draw();
            return;
        }

        for (const auto& group : groups) {
            if (group.name == "HangarMaterial")
                TEXTURES->bindTexture("hangar", GL_TEXTURE0);
            else if (group.name == "HangarAddMaterial")
                TEXTURES->bindTexture("hangar_add", GL_TEXTURE0);
            else if (group.name == "HangarTransparentMaterial")
  ...
  void displayRockets(const std::vector<gameobjects::Rocket> &rockets) {
    ...
    void displayPlaneHealth(float health, float totalTime) {
        ...
        VAOS->bind("quad");
        SHADERS->use("planehealth");
        ShaderProgram &shader = SHADERS->getShader("planehealth");
    ...
    void displayFog() {
        ...
        glEnable(GL_DEPTH_TEST);
        glDisable(GL_CULL_FACE);
        glDepthMask(GL_FALSE); // CRITICAL: Stop the fog from hiding objects behind it

        VAOS->bind("quad");
        SHADERS->use("fog");
        ShaderProgram &fogshader = SHADERS->getShader("fog");

        // Ensure you also pass your view and projection matrices!
        fogshader.uniformMat4x4("persp", window.getPerspective());
        fogshader.uniformMat4x4("view", cam.viewMatrix());

        fogshader.uniformVec3("camPos", cam.position);
        ...
        fogshader.uniformMat4x4("transform", transform);

        VAOS->draw();

        glDepthMask(GL_TRUE); // Re-enable depth writing for the rest of the frame
        glDisable(GL_BLEND);
    }
