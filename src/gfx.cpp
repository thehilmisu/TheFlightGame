...
void Model::dataToBuffers(const std::vector<unsigned int> &buffers) const {
  ...
  // index 2 = normals
  // index 3 = indices
  ASSERT(buffers.size() >= 4, "FAILED!!");

  mesh::Meshf vertdata = vertData(), tcdata = tcData(), normData = normalData();
  // vertex positions
  glBindBuffer(GL_ARRAY_BUFFER, buffers.at(0));
  glBufferData(GL_ARRAY_BUFFER, vertdata.size(), vertdata.ptr(),
               GL_STATIC_DRAW);
  ...
  glBufferData(GL_ARRAY_BUFFER, normData.size(), normData.ptr(),
               GL_STATIC_DRAW);
...
Model createConeModel2(unsigned int prec) {
  ...
  for (int i = 0; i < prec; i++) {
    ...
    glm::vec2 tc =
        glm::vec2(cosf(angle), sinf(angle)) * 0.5f + glm::vec2(0.5f, 0.5f);
...
Model createFrustumModel(unsigned int prec, float radius1, float radius2) {
  ...
  for (int i = 0; i < prec; i++) {
    ...
    if (i < prec / 2)
      ...
      tcx = std::max(1.0f - fract * 2.0f, 0.0f);
    // Calculate normals
    glm::vec3 top = glm::vec3(cosf(angle), 0.0f, sinf(angle)) * radius2 +
  ...
  for (int i = 0; i < prec; i++) {
    ...
    if (i < prec / 2)
      ...
      tcx = std::max(1.0f - fract * 2.0f, 0.0f);
    // Calculate normals
    glm::vec3 bot = glm::vec3(cosf(angle), 0.0f, sinf(angle)) * radius1;
    float nextangle = float(i + 1) / float(prec) * 2.0f * M_PI;
    glm::vec3 nextvert =
