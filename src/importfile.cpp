...
namespace impfile
{
	...
	std::vector<Entry> parseBin(const std::string &content)
	{
		...
		while (std::getline(filecontents, line))
		{
			...
			if (res.isError())
			{
				fprintf(stderr, "parseBin: Quote validation failed on line %d\n", lineCount);
				res.output();
				return entries;
			}
			processed += line;
		...
		// Parse file contents
		while (parsedStream.rdbuf()->in_avail())
		{
			Entry e;
			Result res = parseEntry(e, parsedStream);
			if (res.isError())
			{
				fprintf(stderr, "parseBin: parseEntry failed\n");
				res.output();
				return entries;
			}
			entries.push_back(e);
