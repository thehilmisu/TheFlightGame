...
namespace game {
    ...
    void loadAssets() {
        if (AssetLoader &assetLoader = AssetLoader::getInstance(); !assetLoader.loadAssets("assets.bin"))
    ...
    void generateNewChunks(
        ...
        if (bool generated = decorations.genNewDecorations(cam.position.x, cam.position.z, permutations)) {
    ...
    void initGlobalValUniformBlock() {
        const float viewdist =
                CHUNK_SZ * SCALE * 2.0f * static_cast<float>(RANGE) * std::pow(LOD_SCALE, MAX_LOD - 2);
