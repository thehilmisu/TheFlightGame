...
namespace gameobjects {
    void gobjs::Plane::update(float dt,const UpdateContext &ctx) {
        ...
        if (values.at("shoottimer") < 0.0f &&
            ...
            ctx.bullets->push_back(gobjs::Bullet(transform, speed, glm::vec3(-8.5f, -0.75f, 8.5f)));
            ctx.bullets->push_back(gobjs::Bullet(transform, speed, glm::vec3(8.5f, -0.75f, 8.5f)));
        }
        ...
        //checkIfPlaneCrashed(*ctx.worldseed);
    }

    void gobjs::Plane::checkIfCrashed(const infworld::worldseed &permutations) {
